...
# Computation
results <- perceptron(y, X, eta = 0.05, batch_size = trunc(nrow(X)/2), verbose = TRUE)
...
# second part of the plot with classes computed by the model
colors.vec <- results$fitted
...
# Confusion matrix
conf_matrix <- table(fitted = results$fitted, initial = y)
conf_matrix
